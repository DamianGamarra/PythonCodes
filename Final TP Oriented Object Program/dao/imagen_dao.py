...
class Imagen_DAO(DAO):
    def __init__(self) -> None:
        super().__init__()
        self.nombre_tabla = "imagenes_obras"
    ...
    def insertar_registro(self, objeto, id_obra):
        try:
            db, cursor = self.conectar_bd()
            # Insertar un registro a la tabla
            cursor.execute(f'''INSERT INTO {self.nombre_tabla} (id_obra,descripcion)
                VALUES
                ({id_obra},'{objeto.descripcion}');'''
            )
    ...
    def importar_registro_csv(self, elementos, id_obra):
        try:
            db, cursor = self.conectar_bd()
            # Insertar un registro a la tabla
            cursor.execute(f'''INSERT INTO {self.nombre_tabla} (id_obra,descripcion)
